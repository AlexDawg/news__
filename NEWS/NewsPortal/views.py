...
class PostCreate(PermissionRequiredMixin, CreateView):
   ...
   #
    ...
    #def send_message(instanse, **kwargs):
    #    subscribed_users = []
    #    for category_subscribe in CategorySubscribe.objects.filter(category=instanse.categories.all()):
    #        subscribed_users.append(category_subscribe.subscriber.email)
    #
    #        html_content = render_to_string(
    #            'newpost_mail.html', {'post': post,}
    #        )
    #        send_mail(
    #            subject='Новая статья в категории!',
    #            message=Post.text[:50],
    #            from_email='news.mail@yandex.ru',
    #            recipient_list=['news.mail@yandex.ru', ],
    #            fail_silently=False,
    #        )
    #
    #        post_save.connect(instanse.send_message, sender=Post)
